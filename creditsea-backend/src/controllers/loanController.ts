...
export const createLoanApplication = async (
  ...
  try {
    ...
    if (!applicationData.loanAmount) {
      ...
      res.status(400).json({ message: 'Loan amount is required' });
      return;
    }
...
export const verifyLoanApplication = async (
  ...
  try {
    ...
    if (application.status !== LoanStatus.PENDING) {
      console.log(
        'Loan application is not in pending status. Current status:',
        application.status
      );
...
export const approveLoanApplication = async (
  ...
  try {
    ...
    if (application.status !== LoanStatus.VERIFIED) {
      console.log(
        'Only verified applications can be approved. Current status:',
        application.status
      );
...
export const getDashboardStats = async (
  ...
  try {
    ...
    const totalVerified = await LoanApplication.countDocuments({
      status: LoanStatus.VERIFIED,
    });
    const totalApproved = await LoanApplication.countDocuments({
      status: LoanStatus.APPROVED,
    });
    const totalRejected = await LoanApplication.countDocuments({
    ...
    const totalLoanAmount =
      totalLoanAmountResult.length > 0 ? totalLoanAmountResult[0].total : 0;
